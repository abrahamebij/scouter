...
export default function VaultPanel() {
  ...
  // Track pending tx hash before it gets cleared by the hook
  useEffect(() => {
    if (vault.pendingTx) setLastTx(vault.pendingTx);
  }, [vault.pendingTx]);

  // Listen for on-chain confirmations
  useEffect(() => {
    vault.onTxConfirmed((action) => {
      if (action === "approve") {
        toast("USDC approved — depositing now...", "info");
  ...
  const handleMax = () => {
    if (tab === "deposit" && vault.fmt.userUsdcBalance) {
      setAmount(vault.fmt.userUsdcBalance);
      ...
      <div className="grid grid-cols-2 gap-4 mb-6">
        <div>
          ...
          <div className="font-headline font-bold text-2xl text-primary">
            {vault.fmt.targetLeverage ? `${vault.fmt.targetLeverage}x` : "3.0x"}
          </div>
        </div>
        <div>
          <div className="font-label text-[10px] text-on-surface-variant uppercase tracking-widest mb-1">
          ...
          <div className="font-headline font-bold text-2xl text-secondary">
            {vault.fmt.leverage ? `${vault.fmt.leverage}x` : "—"}
          </div>
        </div>
      </div>
        ...
        {vault.fmt.userShares && parseFloat(vault.fmt.userShares) > 0 && (
          <div className="flex justify-between items-center">
            ...
            <span className="font-label font-bold text-sm text-secondary">
              {parseFloat(vault.fmt.userShares).toFixed(4)} yxSPY
            </span>
          </div>
        )}
      </div>
        ...
        <>
          ...
          <div className="relative mb-3">
            <input
              ...
              placeholder={tab === "deposit" ? "USDC amount" : "yxSPY shares"}
          ...
          <div className="flex items-center gap-1.5 text-[11px] text-on-surface-variant mb-4 px-1">
            {tab === "deposit" ? (
              <>
                <Image src="/coins/usdc.svg" alt="USDC" width={14} height={14} className="rounded-full" unoptimized />
                Balance: {vault.fmt.userUsdcBalance ? `${parseFloat(vault.fmt.userUsdcBalance).toFixed(2)} USDC` : "—"}
              </>
            ) : (
              <>
                <Image src="/xStocks/spy.svg" alt="yxSPY" width={14} height={14} className="rounded-full" unoptimized />
                Shares: {vault.fmt.userShares ? `${parseFloat(vault.fmt.userShares).toFixed(4)} yxSPY` : "—"}
              </>
            )}
          </div>
          ...
          {lastTx && (
            <a
              href={`https://explorer-sepolia.inkonchain.com/tx/${lastTx}`}
              target="_blank"
