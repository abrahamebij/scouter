...
export async function generateMetadata({ params }: DashboardCompanyPageProps): Promise<Metadata> {
  ...
  return {
    title: `${companyName} ($${product.symbol}) | Profile | Scouter`,
    description: `PreStocks token metrics, implied valuation, and benchmark mark pricing for ${companyName} on Solana.`,
  };
}

export default async function DashboardCompanyPage({ params }: DashboardCompanyPageProps) {
  const { symbol } = await params;
  const product = await getPreStockBySymbol(symbol);
  ...
  if (!product) {
    ...
    return (
      <div className="max-w-4xl mx-auto p-4 sm:p-6 lg:p-8 py-20 text-center space-y-6">
          ...
          <div className="pt-4 max-w-lg mx-auto">
            ...
            <div className="flex flex-wrap justify-center gap-2">
              {allSymbols.map((sym) => (
                <Link
                  key={sym}
                  href={`/dashboard/company/${sym.toLowerCase()}`}
  ...
  return (
    <div className="max-w-7xl mx-auto p-4 sm:p-6 lg:p-8 space-y-10">
      ...
      {/* Scout Intelligence Brief & Timeline */}
      ...
      <div className="bg-surface-container-low/60 rounded-2xl border border-outline-variant/20 p-6 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-3 flex-shrink-0">
          <Link
            href={`/dashboard/market/${normalizeSymbol(product.symbol)}`}
          ...
          <Link
            href={`/dashboard/compare?symbols=${normalizeSymbol(product.symbol)}`}
