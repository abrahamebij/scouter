...
interface DashboardMarketPageProps {
  params: Promise<{ symbol: string }>;
}

export const revalidate = 60;

export async function generateMetadata({ params }: DashboardMarketPageProps): Promise<Metadata> {
...
export default async function DashboardMarketTerminalPage({ params }: DashboardMarketPageProps) {
  const { symbol } = await params;
    ...
    <div className="max-w-7xl mx-auto p-4 sm:p-6 lg:p-8 space-y-6">
      ...
      <div className="flex flex-wrap items-center justify-between gap-4 pb-2 border-b border-outline-variant/15">
        ...
        <div className="flex items-center gap-3 text-xs">
          <Link
            href={`/dashboard/company/${normalizeSymbol(product.symbol)}`}
      ...
      {/* Main Terminal Grid: 2/3 Market View, 1/3 Intelligence Chat */}
